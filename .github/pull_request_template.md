...
- [ ] **Host-process impact** — throws, unhandled rejections, timers keeping the process alive, blocking work on evaluation paths (Founding §8.1)
- [ ] **Public API** — `src/index.ts`; backward compatibility is sacred (Founding §8.3)
...
where relevant: a cold start with no connectivity; a cachePath load after a long outage
(expiry unenforced); a revoked key mid-run.
-->

## Testing

<!-- The local gate is `pnpm build && pnpm lint && pnpm test`. What did you verify beyond
it, especially against a live backend, which CI cannot do? -->
